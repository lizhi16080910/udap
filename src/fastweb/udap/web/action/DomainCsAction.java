...
@Namespace("/")
@Action(value = "olddomaincs", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class DomainCsAction extends ActionSupport {
...
//					Map<String,Map<Long,Long>> pnocs = new TreeMap<String,Map<Long,Long>>();
					...
					LongTerms agge = domainBucket.getAggregations().get("timestamp");
					...
					for(Bucket pnoBucket : pnoTerms.getBuckets()){
						...
						LongTerms agge = pnoBucket.getAggregations().get("timestamp");
						Map<Long,Long> timeMap = new TreeMap<Long,Long>();
						for (Bucket bucket : agge.getBuckets()) {
							Sum sum = bucket.getAggregations().get("cs");
							long key = Long.parseLong(bucket.getKey());

							long newKey = key / 300 * 300;
							long value = ((Double) sum.getValue()).longValue();
							if (timeMap.containsKey(newKey)) {
								long newValue = timeMap.get(newKey) + value;
								timeMap.put(newKey, newValue);
	...
	public Map<String, Map<Long,Long>> getResult() {
		...
		for(String domainPnoPair : domainPnoPairs){
			...
			if(!result.containsKey(domain)){
				Map<Long,Long> domainMap = new TreeMap<Long,Long>();
				...
				if(result_temp.containsKey(domain)) {
					...
					for(String pno : pnoArray){
						if(!domainMap.containsKey(pno)){
							Map<Long,Long> timeMap = new TreeMap<Long,Long>();
							//添加时间区间的每五分钟的一个数据，时间戳是每加五分钟的时间戳，value=0
///						timeMap.put(0L, 0L);
							int count = 0;
						while(begin<=end || count <288){
							timeMap.put(begin, 0L);
							begin+=300L;
							count+=1;
						}
