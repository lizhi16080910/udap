 ...
 */
@Namespace("/")
@Action(value = "bossflowsizecountprv", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = {"excludeProperties","message"}),
		   @Result(name = ActionSupport.ERROR, type = "json", params={"includeProperties","message"})})
public class BossFlowSizeCountPrv extends ActionSupport {
		...
		try {
			...
			try {
			  ...
			  //条件
				...
				AbstractAggregationBuilder subbuilder  = AggregationBuilders.terms("platform").field("platform").subAggregation(prvIdFormAgg).size(Integer.MAX_VALUE);
