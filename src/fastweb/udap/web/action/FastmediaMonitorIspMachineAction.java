 ...
 */
@Namespace("/")
@Action(value = "fastmediamonitorispmachine", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class FastmediaMonitorIspMachineAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			BoolQueryBuilder allQuery = QueryBuilders.boolQuery().must(timestampQuery).must(QueryBuilders.termQuery("prv", this.prv));
			...
			AbstractAggregationBuilder platformKey = AggregationBuilders.terms("platform").field("platform").subAggregation(statusKey).size(Integer.MAX_VALUE);
			AbstractAggregationBuilder hostnameKey = AggregationBuilders.terms("hostname").field("hostname").subAggregation(platformKey).size(Integer.MAX_VALUE);
			AbstractAggregationBuilder machineKey = AggregationBuilders.terms("machine").field("machine").subAggregation(hostnameKey).size(Integer.MAX_VALUE);
			AbstractAggregationBuilder ispKey = AggregationBuilders.terms("isp").field("isp").subAggregation(machineKey).size(Integer.MAX_VALUE);
			...
			for (Bucket bucket : ispAgge.getBuckets()) {
				...
				for(Bucket machineBucket : machineAgge.getBuckets()){
					...
					for (Bucket hostnameBucket : hostnameAgge.getBuckets()) {
						...
						/*
						StringTerms statusAgge = hostnameBucket.getAggregations().get(params);
						Map<String, Long> map = new TreeMap<String, Long>();
						for (Bucket sta : statusAgge.getBuckets()) {
							String key = sta.getKey();
							Sum count = sta.getAggregations().get("count");
				...
				result.put(bucket.getKey(), machinemap);
			}
