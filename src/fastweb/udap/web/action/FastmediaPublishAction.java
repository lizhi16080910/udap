 ...
 */
@Namespace("/")
@Action(value = "fastmediapublish", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class FastmediaPublishAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			AbstractAggregationBuilder domainKey = AggregationBuilders.terms("domain").field("domain").subAggregation(timestampKey).size(100);
			...
			for (Bucket domainBucket : domainTerms.getBuckets()) {
				...
				result.put(domainBucket.getKey(), timeMap);
			}
