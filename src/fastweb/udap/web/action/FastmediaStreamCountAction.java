 ...
 */
@Namespace("/")
@Action(value = "fastmediastreamcount", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class FastmediaStreamCountAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			AbstractAggregationBuilder statusKey = AggregationBuilders.terms(params).field(params).subAggregation(cSum).size(200);
			AbstractAggregationBuilder streamKey = AggregationBuilders.terms("stream").field("stream").subAggregation(statusKey).size(200);

			searchRequestBuilder = client.prepareSearch(INDEX).setQuery(allQuery).setFrom(0).setSize(0).addAggregation(streamKey);
			log.info("Query Url: " + ServletActionContext.getRequest().getRemoteAddr() + ": " + searchRequestBuilder.toString());

			SearchResponse response = searchRequestBuilder.execute().actionGet();
			
			StringTerms streamAgge = response.getAggregations().get("stream");
			for (Bucket streamBucket : streamAgge.getBuckets()) {
				StringTerms statusAgge = streamBucket.getAggregations().get(params);
				...
				result.put(streamBucket.getKey(), params);
			}
