...
public class CdnlogISMSMonitorThread implements Runnable {
	...
	public CdnlogISMSMonitorThread() {
	}

	@Override
	public void run() {
		Queue<Isms> queue = CdnlogISMSMonitorQueryAction.getQueue();
		...
		/* 恢复静态变量 可并行执行的线程数 */
		CdnlogISMSMonitorQueryAction.getQueryCntThread().incrementAndGet();
	...
	private String queryString(Isms isms) {
		...
		StringBuilder sql = new StringBuilder("insert into table cdnlog_isms_monitor partition(id='" + isms.getId() + "', commandId='" + isms.getCommandId() + "') select domain,ip,node,count(*) as count, min(timestmp) as timestmp,url from ");
	...
	public static void setPool(ExecutorService pool) {
		CdnlogISMSMonitorThread.pool = pool;
	}
}
