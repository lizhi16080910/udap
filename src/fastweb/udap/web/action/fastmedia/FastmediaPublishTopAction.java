...
/**
 * 流媒体数据监控 域名主播数排行top10
 */
@Namespace("/")
@Action(value = "fastmediapublishtop", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class FastmediaPublishTopAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			AbstractAggregationBuilder cSum = AggregationBuilders.sum("publish").field("publish");
			AbstractAggregationBuilder statusKey = AggregationBuilders.terms("domain").field("domain").subAggregation(cSum).size(10).order(Terms.Order.aggregation("publish", false));
			...
			for (Bucket domainBucket : timestampAgge.getBuckets()) {
				...
				for (Bucket sta : domainAgge.getBuckets()) {
					...
					Sum count = sta.getAggregations().get("publish");
