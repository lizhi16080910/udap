 ...
 */
@Namespace("/")
@Action(value = "cdnloguacount", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class CdnlogUaCountAction extends ActionSupport {
	private static final long serialVersionUID = 1L;

	private final Log log = LogFactory.getLog(getClass());

	public static final String INDEX = "cdnlog.ua.count";
	...
	public String execute() throws Exception {
		...
		try {
			...
			if (domain.equalsIgnoreCase("all") && StringUtils.isNotBlank(userid)) {
			...
			AbstractAggregationBuilder uaKey = AggregationBuilders.terms("ua").field("ua").subAggregation(countSum);

			searchRequestBuilder = client.prepareSearch(INDEX).setQuery(allQuery).setFrom(0).setSize(0).addAggregation(uaKey);
			log.info("Query Url: " + ServletActionContext.getRequest().getRemoteAddr() + ": " + searchRequestBuilder.toString());

			SearchResponse response = searchRequestBuilder.execute().actionGet();

			StringTerms uaAgge = response.getAggregations().get("ua");

			for (Bucket uabucket : uaAgge.getBuckets()) {
				String key = uabucket.getKey();
				Sum count = uabucket.getAggregations().get("count");
