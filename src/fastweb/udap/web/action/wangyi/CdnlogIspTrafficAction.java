 ...
 */
@Namespace("/")
@Action(value = "cdnlogisptraffic", results = { @Result(name = ActionSupport.SUCCESS, type = "json", params = { "excludeProperties", "message" }), @Result(name = ActionSupport.ERROR, type = "json", params = { "includeProperties", "message" }) })
public class CdnlogIspTrafficAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		try {
			...
			AbstractAggregationBuilder ispKey = AggregationBuilders.terms("isp").field("isp").subAggregation(trafficSum);
			...
			for (Bucket ispbucket : ispAgge.getBuckets()) {
				String key = ispbucket.getKey();
				Sum traffic = ispbucket.getAggregations().get("traffic");
