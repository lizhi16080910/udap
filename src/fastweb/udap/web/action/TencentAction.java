...
public class TencentAction extends ActionSupport {
	public static final String INDEX = "tencent";
		...
		try {
			...
			try {
				...
				AbstractAggregationBuilder esSum = AggregationBuilders
						.sum("es").field("es");
				String[] termBucketsName ={"isp","dir","timestamp","domain"};
				
				TermsBuilder domainAgg = AggregationBuilders.terms("statusCode").field("statusCode").subAggregation(countSum).size(Integer.MAX_VALUE);
				for(String bucketName : termBucketsName) {
					TermsBuilder termAgg = AggregationBuilders.terms(bucketName).field(bucketName);
					if(bucketName.equalsIgnoreCase("isp")) {
						termAgg = termAgg.subAggregation(csSum).subAggregation(esSum).subAggregation(countSum).subAggregation(domainAgg).size(Integer.MAX_VALUE);
					}  else {
